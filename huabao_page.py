...
def dropcreate_huabao_transactions():
    with conn.cursor() as cur:
        ...
        cur.execute(
        'CREATE TABLE IF NOT EXISTS huabao_transactions_{}(DATE VARCHAR(255),TIME VARCHAR(255),CODE VARCHAR(255),name VARCHAR(255),type VARCHAR(255),amount DOUBLE)'.format(st.session_state['user']))
...
def load_and_save_transaction_file(files_list):
    ...
    transactions_df_list = [pd.read_excel(file, dtype={'证券代码': str,'成交日期':str,'发生金额':float})[['成交日期','成交时间','证券代码','证券名称','委托类别','发生金额']] for file in files_list]
    for i in range(len(transactions_df_list)):
        transactions_df_list[i].columns = ['DATE','TIME','CODE','NAME','TYPE','AMOUNT']
        ...
        transactions_df_list[i] = transactions_df_list[i].set_index(['DATE','TIME','CODE','NAME','TYPE'])
    ...
    if 'HUABAO_TRANSACTIONS_'+st.session_state['user'].upper() in table_list:
        existing_transactions_df = utils.load_snowflake_to_pandas('HUABAO_TRANSACTIONS_'+st.session_state['user'].upper(),conn).set_index(['DATE','TIME','CODE','NAME','TYPE'])
...
def display_all_stocks(transactions_df,assets_df,code2name):
    ...
    for stock_code in unique_stock_code1:
        ...
        output.append((stock_code,asset_date,asset_value,xirr))

    sorted_output = sorted(output,key=lambda x:x[2],reverse=True)
    for o in sorted_output:
        stock_code,asset_date,asset_value,xirr = o[0],o[1],o[2],o[3]
